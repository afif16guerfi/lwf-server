...
const { blankData, SEASON, ACCOUNT_STATUS, getAccountStatus } = require("../schema");
...
function publicUser(u) {
  return { id: u.id, role: u.role, username: u.username, email: u.email, fullName: u.fullName, accountStatus: getAccountStatus(u) };
}
...
router.post("/signup", async (req, res) => {
  try {
    ...
    if (nowMember) {
      realtime.sendToUsers(preExistingMemberIds, "conversation:member_joined", { conversationId: publicConv.id, userId: newUser.id, fullName: newUser.fullName });
    }
