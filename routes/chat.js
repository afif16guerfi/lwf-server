...
const { ACCOUNT_STATUS, getAccountStatus, getUserPhotoUrl } = require("../schema");
...
function roleFlags(conv, userId, userRole) {
  ...
  // The public referees' chat has no owner (ownerId is always null — see
  // ensurePublicConversation) and is administered by the admin only, never
  // by a referee "owner". Renaming/adding/removing members there is
  // therefore admin-only, and — unlike a regular group — never available
  // to a non-admin, since there is no group owner role on this
  // conversation to grant it to.
  const isPublic = conv.type === "public";
  const isOwner = isGroup && conv.ownerId === userId;
  return {
    canRename: (isGroup && (isAdmin || isOwner)) || (isPublic && isAdmin),
    canEditPhoto: isGroup && (isAdmin || isOwner),
    canAddMembers: (isGroup && (isAdmin || isOwner)) || (isPublic && isAdmin),
    canRemoveMembers: (isGroup && (isAdmin || isOwner)) || (isPublic && isAdmin),
...
function serializeConversation(conv, data, userId, userRole) {
  ...
  if (conv.type === "private") {
    ...
    name = otherUser ? otherUser.fullNameAr : "محادثة خاصة";
    otherUserRole = otherUser ? otherUser.role : null;
  }
  ...
  return {
    ...
    name,
    avatarLabel,
    photoUrl: conv.type === "private" && otherUserId
      ? getUserPhotoUrl(data, otherUserId)
      : (conv.type === "group" ? conv.photoUrl || null : null),
    otherUserRole,
...
function serializeMember(m, data) {
  ...
  return {
    userId: m.userId,
    fullNameAr: u ? u.fullNameAr : "مستخدم محذوف",
    photoUrl: u ? getUserPhotoUrl(data, u.id) : null,
    role: m.role,
  ...
  const options = poll.options.map((o) => {
    ...
    return {
      ...
      voters: anonymous ? [] : voterIds.map((uid) => {
        ...
        return { id: uid, fullNameAr: u ? u.fullNameAr : "مستخدم محذوف", photoUrl: u ? getUserPhotoUrl(data, u.id) : null, role: u ? u.role : null };
      }),
    };
  });
  return {
...
function serializeReactions(reactions, data, viewerId) {
  ...
  Object.entries(reactions || {}).forEach(([userId, r]) => {
    ...
    groups[r.emoji].push({ userId, fullNameAr: u ? u.fullNameAr : "مستخدم محذوف", photoUrl: u ? getUserPhotoUrl(data, u.id) : null, role: u ? u.role : null });
  });
...
function serializeMessage(m, data, viewerId) {
  ...
  return {
    ...
    senderId: m.senderId,
    senderName: sender ? sender.fullNameAr : "مستخدم محذوف",
...
router.get("/directory", requireActiveAccount, async (req, res) => {
  ...
  const referees = data.users
    ...
    .map((u) => ({ id: u.id, fullNameAr: u.fullNameAr, photoUrl: getUserPhotoUrl(data, u.id), username: u.username }));
  res.json({ referees });
});
...
router.get("/referees", requireActiveAccount, async (req, res) => {
  ...
  const referees = data.users.filter((u) => u.role === "referee" && u.id !== req.user.id).map((u) => {
    ...
    return { id: u.id, fullNameAr: u.fullNameAr, photoUrl: getUserPhotoUrl(data, u.id), username: u.username, conversationId: priv ? priv.id : null };
  });
  res.json({ referees });
});
...
router.get("/conversations/:id/candidates", async (req, res) => {
  ...
  if ((conv.type !== "group" && conv.type !== "public") || !flags.canAddMembers) {
    return res.status(403).json({ error: "غير مصرح لك بإضافة أعضاء." });
  }
  const memberIds = new Set(memberIdsOf(data, conv.id));
  const candidates = data.users
    .filter((u) => u.role === "referee" && !memberIds.has(u.id) && (conv.type !== "public" || getAccountStatus(u) === ACCOUNT_STATUS.ACTIVE))
    .map((u) => ({ id: u.id, fullNameAr: u.fullNameAr, photoUrl: getUserPhotoUrl(data, u.id), username: u.username }));
  res.json({ candidates });
});
...
// A separate step from group creation on purpose (same pattern as document
// uploads in applications.js): the group needs to exist and have an id
// before Cloudinary needs a public_id to key the upload to, and it lets the
// exact same endpoint serve both "set a photo while creating a group" (the
// client creates the group first, then immediately calls this) and "change
// an existing group's photo later" from group info.
router.post("/groups/:id/photo", upload.single("file"), async (req, res) => {
  if (!req.file) return res.status(400).json({ error: "لم يتم إرفاق صورة." });
  if (!req.file.mimetype.startsWith("image/")) {
    return res.status(400).json({ error: "الرجاء اختيار ملف صورة." });
  }
  if (!cloudinaryLib.isConfigured()) {
    return res.status(500).json({ error: "لم يتم إعداد تخزين الملفات (Cloudinary)." });
  }
  const data = await db.getAll();
  const conv = findConversation(data, req.params.id);
  if (!conv || conv.type !== "group") return res.status(404).json({ error: "المجموعة غير موجودة." });
  const flags = roleFlags(conv, req.user.id, req.user.role);
  if (!flags.canEditPhoto) return res.status(403).json({ error: "غير مصرح لك بتغيير صورة هذه المجموعة." });

  let uploadResult;
  try {
    // The client already cropped this down to exactly the circular region
    // the person chose (see cropToCircleBlob() in chat.js) and sends it as
    // a fixed-size square PNG, so no server-side transform is needed here —
    // Cloudinary just stores the finished image as-is.
    uploadResult = await cloudinaryLib.uploadBuffer(req.file.buffer, {
      public_id: `group_${conv.id}_${uuidv4().slice(0, 8)}`,
      resource_type: "image",
    });
  } catch (e) {
    console.error(e);
    return res.status(502).json({ error: "تعذّر رفع الصورة. حاول مرة أخرى." });
  }

  if (conv.photoPublicId) {
    await cloudinaryLib.destroyAsset(conv.photoPublicId, "image");
  }
  conv.photoUrl = uploadResult.secure_url;
  conv.photoPublicId = uploadResult.public_id;
  conv.updatedAt = nowIso();
  await db.saveAll(data);
  realtime.sendToUsers(memberIdsOf(data, conv.id), "group:photo_changed", { conversationId: conv.id, photoUrl: conv.photoUrl });
  res.json({ conversation: serializeConversation(conv, data, req.user.id, req.user.role) });
});

// ---- Remove group photo (revert to the 👥 icon) ----
router.delete("/groups/:id/photo", async (req, res) => {
  ...
  if (conv.photoPublicId) {
    if (cloudinaryLib.isConfigured()) await cloudinaryLib.destroyAsset(conv.photoPublicId, "image");
    conv.photoPublicId = null;
  }
  conv.photoUrl = null;
  conv.updatedAt = nowIso();
  await db.saveAll(data);
  realtime.sendToUsers(memberIdsOf(data, conv.id), "group:photo_changed", { conversationId: conv.id, photoUrl: null });
  res.json({ conversation: serializeConversation(conv, data, req.user.id, req.user.role) });
});

// ---- Rename group (also used by the admin to rename the public referees'
//      chat — see roleFlags: admin-only there, no group "owner" concept) ----
router.put("/groups/:id", async (req, res) => {
  const { name } = req.body;
  if (!name || !String(name).trim()) return res.status(400).json({ error: "يرجى إدخال اسم المجموعة." });
  const data = await db.getAll();
  const conv = findConversation(data, req.params.id);
  if (!conv || (conv.type !== "group" && conv.type !== "public")) return res.status(404).json({ error: "المجموعة غير موجودة." });
  const flags = roleFlags(conv, req.user.id, req.user.role);
...
router.delete("/groups/:id", async (req, res) => {
  ...
  if (conv.photoPublicId && cloudinaryLib.isConfigured()) {
    await cloudinaryLib.destroyAsset(conv.photoPublicId, "image");
  }
...
// ---- Add members (also used by the admin to add referees back to the
//      public chat — see roleFlags) ----
router.post("/groups/:id/members", requireActiveAccount, async (req, res) => {
  ...
  // On the public chat, only currently-active referees are eligible — a
  // pending/needs-edit/rejected account still can't access it either way
  // (see canAccessConversation), so adding one here would be a no-op that
  // just looks like a bug in the member list.
  const toAdd = Array.isArray(memberIds)
    ? [...new Set(memberIds)].filter((id) => {
        if (existing.has(id)) return false;
        const u = data.users.find((u) => u.id === id && u.role === "referee");
        if (!u) return false;
        if (conv.type === "public" && getAccountStatus(u) !== ACCOUNT_STATUS.ACTIVE) return false;
        return true;
      })
    : [];
  ...
  // Re-adding someone to the public chat clears any earlier admin removal —
  // otherwise ensurePublicConversation() would never re-enrol them and the
  // membership row we just pushed would immediately look "stuck".
  if (conv.type === "public" && Array.isArray(conv.excludedUserIds) && conv.excludedUserIds.length) {
    conv.excludedUserIds = conv.excludedUserIds.filter((id) => !toAdd.includes(id));
  }
  conv.updatedAt = iso;
  await db.saveAll(data);
...
// ---- Remove a member (also used by the admin to remove a referee from the
//      public chat — see roleFlags). Removing from the public chat records
//      the exclusion on the conversation so the referee isn't silently
//      re-enrolled the next time ensurePublicConversation() runs (see
//      chatCore.js); the admin can undo it any time via "add members". ----
router.delete("/groups/:id/members/:userId", async (req, res) => {
  ...
  if (conv.type === "public") {
    if (!Array.isArray(conv.excludedUserIds)) conv.excludedUserIds = [];
    if (!conv.excludedUserIds.includes(req.params.userId)) conv.excludedUserIds.push(req.params.userId);
  }
  conv.updatedAt = nowIso();
  await db.saveAll(data);
...
router.delete("/conversations/:id", async (req, res) => {
  ...
  // edit, or rejected) can only ever reach the admin through this one
  // private conversation — never let them delete it, or they'd have no way
  // left to contact the admin at all.
  const caller = findUser(data, req.user.id);
  if (caller && caller.role === "referee" && getAccountStatus(caller) !== ACCOUNT_STATUS.ACTIVE) {
...
router.post("/conversations/:id/typing", async (req, res) => {
  ...
  realtime.sendToUsers(effectiveMemberIdsOf(data, conv.id), "typing", { conversationId: conv.id, userId: req.user.id, fullNameAr: me ? me.fullNameAr : "" }, req.user.id);
  res.json({ ok: true });
});
