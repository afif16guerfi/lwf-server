...
const { FIELD_GROUPS, DOC_TYPES, allFieldKeys, SEASON } = require("../schema");
...
// ---- Get my application ----
router.get("/mine", requireAuth, requireRole("referee"), async (req, res) => {
...
router.put("/mine", requireAuth, requireRole("referee"), async (req, res) => {
  const data = await db.getAll();
  const app = findApp(data, req.user.id);
  if (!app) return res.status(404).json({ error: "لا يوجد ملف انخراط." });
...
// ---- Upload a document ----
router.post("/mine/documents/:docKey", requireAuth, requireRole("referee"), upload.single("file"), async (req, res) => {
  const { docKey } = req.params;
  if (!docKeys().includes(docKey)) return res.status(400).json({ error: "نوع وثيقة غير معروف." });
...
router.post("/mine/submit", requireAuth, requireRole("referee"), async (req, res) => {
  const data = await db.getAll();
  const app = findApp(data, req.user.id);
  if (!app) return res.status(404).json({ error: "لا يوجد ملف انخراط." });
  ...
  const missingDocs = DOC_TYPES.filter((d) => !app.documents[d.key]).map((d) => d.label);
