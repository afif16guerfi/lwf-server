...
router.post("/mine", requireAuth, requireRole("referee"), upload.single("attachment"), async (req, res) => {
...
router.put("/mine/:id", requireAuth, requireRole("referee"), upload.single("attachment"), async (req, res) => {
  const data = await db.getAll();
...
router.delete("/mine/:id", requireAuth, requireRole("referee"), async (req, res) => {
  const data = await db.getAll();
  const request = findRequest(data, req.params.id);
  if (!request || request.userId !== req.user.id) return res.status(404).json({ error: "الطلب غير موجود." });
