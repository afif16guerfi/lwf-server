...
// Forcibly closes every open socket for a user — used when an account is
// disabled or deleted (see routes/admin.js) so access is cut immediately
// instead of waiting for their token to expire or their next request.
function disconnectUser(userId) {
  const set = clients.get(userId);
  if (!set) return;
  for (const ws of set) {
    try { ws.close(4001, "account_disabled"); } catch (e) { /* already closing */ }
  }
  clients.delete(userId);
}

module.exports = { register, unregister, isOnline, sendToUser, sendToUsers, disconnectUser };
