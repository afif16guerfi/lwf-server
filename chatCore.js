...
function canAccessConversation(data, conversationId, userId) {
  ...
  if (conv && conv.type === "public") {
    ...
    if (user && user.role === "referee" && getAccountStatus(user) === ACCOUNT_STATUS.PENDING) return false;
  }
  return true;
}
...
function effectiveMemberIdsOf(data, conversationId) {
  ...
  return ids.filter((uid) => {
    ...
    return !(user && user.role === "referee" && getAccountStatus(user) === ACCOUNT_STATUS.PENDING);
  });
}
