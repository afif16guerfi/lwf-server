...
(function(){
  ...
  // A referee whose account hasn't been activated yet can only reach the
  // admin through one private conversation — the UI never offers to delete
  // it (the backend refuses it too, see routes/chat.js DELETE /conversations/:id).
  function isPendingReferee(){ return !!(ST.session && ST.session.user.role === "referee" && ST.session.user.accountStatus === "pending"); }
  ...
  function avatarHtml(c){
    if(c.type === "public") return `<div class="chat-avatar public">📢</div>`;
    if(c.type === "group") return `<div class="chat-avatar group">👥</div>`;
    return `<div class="chat-avatar${c.online ? ' online' : ''}">${escapeHtml(initial(c.name))}</div>`;
  }
  ...
  function showTyping(payload){
    ...
    if(payload.conversationId === ST.currentId){
      ...
      const sub = document.getElementById("chat-typing-line");
      if(sub) sub.textContent = `${payload.fullName} يكتب الآن...`;
    }
    const conv = ST.conversations.find(c=>c.id===payload.conversationId);
    if(conv){ conv._typingLabel = payload.fullName; renderSidebar(); }
  }
  ...
  function renderMemberPicker(containerId, list, checkedIds){
    ...
    el.innerHTML = list.map((u)=>`
      <label class="chat-picker-row">
        ...
        <span class="chat-avatar sm">${escapeHtml(initial(u.fullName))}</span>
  ...
  async function openNewGroupModal(){
    openModal(`
      <div class="chat-modal">
        ...
        <div class="chat-modal-body">
          <div class="field"><label>اسم المجموعة</label><input type="text" id="chat-newgroup-name" placeholder="مثال: حكام الدرجة الأولى"></div>
          <div class="field">
            <label>الأعضاء</label>
    ...
    try{
      ...
      renderMemberPicker("chat-newgroup-list", referees, []);
    }catch(e){
  ...
  function memberRowHtml(m, c){
    const canRemove = c.permissions.canRemoveMembers && c.type === "group" && m.userRole !== "admin" && m.userId !== ST.session.user.id;
    return `<div class="chat-member-row" data-member-row="${m.userId}">
      <span class="chat-avatar sm${m.online ? ' online' : ''}">${escapeHtml(initial(m.fullName))}</span>
      <div class="chat-member-info">
        <span class="chat-member-name">${escapeHtml(m.fullName)}${m.userRole==='admin' ? ' <span class="chat-admin-tag">الإدارة</span>' : ''}${(m.role==='owner' && m.userRole!=='admin') ? ' <span class="chat-owner-tag">منشئ المجموعة</span>' : ''}</span>
    ...
    openModal(`
      <div class="chat-modal">
        ...
        <div class="chat-modal-body">
          ...
          ${(c.permissions.canAddMembers && c.type==='group') ? `<button type="button" class="btn btn-outline btn-sm mt-8" data-chat-action="open-addmembers">+ إضافة أعضاء</button>` : ""}
        </div>
      </div>`);
  }
  ...
  async function removeMemberConfirm(userId){
    ...
    if(!confirm(`هل تريد إزالة "${m ? m.fullName : ''}" من المجموعة؟`)) return;
    try{
      ...
      <div class="chat-poll-voters-group">
        ...
        ${opt.voters.length ? opt.voters.map((v)=>`
          <div class="chat-member-row">
            <span class="chat-avatar sm">${escapeHtml(initial(v.fullName))}</span>
            <span class="chat-member-name">${escapeHtml(v.fullName)}</span>
    ...
    const body = msg.reactions.map((r)=>`
      <div class="chat-poll-voters-group">
        ...
        ${r.users.map((u)=>`
          <div class="chat-member-row">
            <span class="chat-avatar sm">${escapeHtml(initial(u.fullName))}</span>
            <span class="chat-member-name">${escapeHtml(u.fullName)}</span>
          </div>`).join("")}
      </div>`).join("");
    openModal(`
      <div class="chat-modal">
  ...
  function onShellClick(e){
    ...
    switch(action){
      ...
      case "open-newgroup": closeAllDropdowns(); openNewGroupModal(); break;
  ...
  function shellHtml(session){
    ...
    const canStartNewChats = isAdmin || session.user.accountStatus !== "pending";
    return `
    <div class="chat-page">
  ...
  async function mount(session, routeParam){
    ...
    if(session.user.role === "referee"){
      try{
        const { conversation } = session.user.accountStatus === "pending"
        ...
        if(session.user.accountStatus === "pending"){ selectConversation(conversation.id); return; }
      }catch(e){}
    }
