...
(function(){
    ...
    const rows = announcements.map(a => `
      <tr data-status="${a.status}" data-search="${escapeHtml((a.title+" "+a.summary).toLowerCase())}">
        ...
        <td class="announce-admin-actions">
          ...
          <a href="#/admin-announcement-stats/${a.id}" class="btn btn-outline btn-sm">📊 إحصائيات القراءة</a>
        ...
        <form id="ann-form" data-id="${a ? a.id : ''}">
          ...
          <div class="field">
            <label style="display:flex;align-items:center;gap:8px;cursor:pointer;">
              <input type="checkbox" name="notifyOnUpdate" value="true" style="width:auto;">
              إرسال إشعار بالتعديل لجميع الحكام المستهدفين (استخدمه فقط إذا كان التعديل جوهريًا)
            </label>
          </div>` : ""}
  ...
  function mount(seg, id, session){
    ...
    if(seg === "admin-announcement-stats"){
      const search = document.getElementById("stats-search");
      if(search) search.addEventListener("input", applyStatsFilter);
      document.querySelectorAll('[data-action="stats-filter"]').forEach(el=>{
        el.addEventListener("click", ()=>{
          ST.statsFilter = el.getAttribute("data-filter");
          go(`admin-announcement-stats/${id}`);
        });
      });
      applyStatsFilter();
    }
  ...
  function attachEditFormHandlers(routeId){
    ...
    form.addEventListener("submit", async (e)=>{
      ...
      const notifyCheckbox = form.querySelector('[name="notifyOnUpdate"]');
      if(notifyCheckbox && notifyCheckbox.checked) fd.append("notifyOnUpdate", "true");
  ...
  /* ============================================================
     ADMIN: read-tracking stats for one announcement (من قرأ / لم يقرأ)
     ============================================================ */
  async function adminStatsPage(id){
    let stats;
    try{
      stats = await api(`/admin/announcements/${id}/read-stats`);
    }catch(e){
      return `<div class="page"><div class="error-msg">${escapeHtml(e.message)}</div></div>`;
    }
    ST.statsRows = stats.rows;

    const rows = stats.rows.map(r => `
      <tr data-read="${r.read ? "read" : "unread"}" data-search="${escapeHtml((r.fullNameAr+" "+r.username).toLowerCase())}">
        <td>${escapeHtml(r.fullNameAr)}</td>
        <td>${escapeHtml(r.username)}</td>
        <td><span class="status-chip ${r.read ? "approved" : "pending"}">${r.read ? "👁️ تمت القراءة" : "⏳ لم يقرأ"}</span></td>
    ...
    return `<div class="page">
      <a href="#/admin-announcements" class="btn btn-ghost btn-sm">↩ العودة إلى قائمة الإعلانات</a>
      <div class="panel mt-16">
        <div class="panel-header"><h3>📊 إحصائيات القراءة — ${escapeHtml(stats.announcement.title)}</h3></div>
        <div class="steps-strip" style="grid-template-columns:repeat(auto-fit,minmax(140px,1fr));">
          <div class="step-mini"><div class="num" style="background:var(--ink-soft);">${stats.targetedCount}</div><h4>👥 عدد المستهدفين</h4></div>
          <div class="step-mini"><div class="num" style="background:var(--green-deep);">${stats.readCount}</div><h4>👁️ تمت القراءة</h4></div>
          <div class="step-mini"><div class="num" style="background:var(--amber);">${stats.unreadCount}</div><h4>⏳ لم تتم القراءة</h4></div>
          <div class="step-mini"><div class="num" style="background:#1d5b93;">${stats.readRate}%</div><h4>📊 نسبة القراءة</h4></div>
        </div>

        <div class="filter-strip" style="display:flex;gap:10px;flex-wrap:wrap;margin:16px 0;">
          ${[["all","الكل"],["unread","لم يقرأ"],["read","قرأ"]].map(([k,label])=>`
            <div class="pill ${ST.statsFilter===k?'status-ok':''}" style="cursor:pointer;" data-action="stats-filter" data-filter="${k}">${label}</div>
          `).join("")}
        </div>
        <div class="field" style="max-width:320px;">
          <input type="text" id="stats-search" placeholder="🔍 ابحث عن اسم حكم...">
        </div>

        ${stats.rows.length===0 ? `<div class="empty"><div class="icon">👥</div><h3>لا يوجد حكام مستهدفون بهذا الإعلان حاليًا</h3></div>` : `
        <div class="table-wrap mt-16">
          <table id="stats-table">
            <thead><tr><th>الحكم</th><th>اسم المستخدم</th><th>الحالة</th><th>تاريخ القراءة</th></tr></thead>
            <tbody>${rows}</tbody>
          </table>
        </div>
        <div class="empty" id="stats-empty" style="display:none;"><div class="icon">🔍</div><h3>لا توجد نتائج مطابقة</h3></div>`}
      </div>
    </div>`;
  }
  ...
  function applyStatsFilter(){
    ...
    let visible = 0;
    rows.forEach(row=>{
      const matchesFilter = ST.statsFilter === "all" || row.getAttribute("data-read") === ST.statsFilter;
      const matchesQuery = !query || (row.getAttribute("data-search")||"").includes(query);
  ...
  window.LWFAnnouncements = { listPage, detailPage, adminListPage, adminEditPage, adminStatsPage, refreshNav, mount };
